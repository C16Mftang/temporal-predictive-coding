...
# load data
X, X_test = get_rotating_mnist('./data', seq_len, seed, device)
size = X.shape
flattened_size = size[-1]*size[-2]*size[-3]
xs = X.reshape(-1, flattened_size)
test_xs = X_test.reshape(-1, flattened_size)
...
inf_iters = 5000 
inferred_xs = torch.zeros_like(xs).to(device)
inferred_xs[:n_cued] = test_xs[:n_cued]
init_inferred_xs = inferred_xs.detach().clone()
prev_z = model.init_hidden().to(device)
hidden_states = []
for k in range(seq_len):
    if k + 1 > n_cued:
        # update the observation layer if we are beyond the point where the true image is given
        model.inference(inf_iters, inf_lr, inferred_xs[k:k+1], us[k:k+1], prev_z, sparse_penal, update_x=True)
    else:
        # do not update if the true image is given
        model.inference(inf_iters, inf_lr, inferred_xs[k:k+1], us[k:k+1], prev_z, sparse_penal, update_x=False)
...
plt.legend()
plt.savefig(result_path + f'/losses_len{seq_len}', dpi=150)

fig, ax = plt.subplots(3, seq_len, figsize=(seq_len, 3))
for i in range(seq_len):
    ax[0, i].imshow(to_np(xs[i]).reshape(28, 28))
    ax[0, i].axis('off')
    ax[1, i].imshow(to_np(init_inferred_xs[i]).reshape(28, 28))
    ax[1, i].axis('off')
    ax[2, i].imshow(to_np(inferred_xs[i]).reshape(28, 28))
    ax[2, i].axis('off')
plt.savefig(result_path + f'/predicted_samples_len{seq_len}_inf{inf_iters}', dpi=150)

fig, ax = plt.subplots(2, seq_len, figsize=(seq_len, 2))
for i in range(seq_len):
    ax[0, i].imshow(to_np(X[i]).reshape(28, 28))
    ax[0, i].axis('off')
    ax[1, i].imshow(to_np(X_test[i]).reshape(28, 28))
    ax[1, i].axis('off')
